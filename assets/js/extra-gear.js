jQuery(document).ready(function ($) {
    ...
    $(".add-row").on("click", function () {
        ...
        if (srsExtraGear.rentingOptions && srsExtraGear.rentingOptions.length > 0) {
            rentingHtml = `<select class="form-select" name="srs_extra_gear[gears][${index}][renting_options][]" multiple>`;
        ...
        <tr class="gear-row" data-gear-index="${index}">
            <td>
                <input type="text" class="form-control" name="srs_extra_gear[gears][${index}][name]">
                <input type="hidden" name="srs_extra_gear[gears][${index}][product_id]" value="0">
            </td>
            <td>
            ...
            <td>
                <input type="number" step="0.01" class="form-control"
                       name="srs_extra_gear[gears][${index}][prices][extra]">
            </td>
            <td>${rentingHtml}</td>
            <td>
    ...
    $(document).on("click", ".add-day", function () {
        ...
        <tr>
            <td><input type="number" min="1" class="form-control"
                       name="srs_extra_gear[gears][${gearIndex}][prices][day][]"></td>
            <td><input type="number" step="0.01" class="form-control"
                       name="srs_extra_gear[gears][${gearIndex}][prices][value][]"></td>
    ...
    function reindexGears() {
        $("#extra-gear-table > tbody > tr.gear-row").each(function (i) {
            ...
            // Update inputs inside this gear row
            $(this).find("input, select").each(function () {
                let name = $(this).attr("name");
                if (name) {
                    name = name.replace(/srs_extra_gear\[gears]\[\d+]/, `srs_extra_gear[gears][${i}]`);
                    $(this).attr("name", name);
                }
            });
        });
