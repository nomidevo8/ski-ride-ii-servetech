...
class SRS_Socks {
    ...
    public function register_settings() {
        register_setting('srs_socks_group', $this->option_key, [
    ...
    // Use the universal sanitizer for gears
        $output = SRS_Sanitize_Settings::sanitize($input, 'socks');

        // Sync products to WooCommerce automatically
        if (!empty($output['socks'])) {
            foreach ($output['socks'] as &$item) {
                $item['product_id'] = SRS_Sync_WooCommerce::sync_product($item, 'socks');
            }
        }
    ...
    public function render_page() {
        ...
        <div class="wrap bootstrap-wrapper">
            <h1 class="wp-heading-inline"><?php _e('Socks', 'ski-ride-servetech'); ?></h1>
            <form method="post" action="options.php">
                <?php settings_fields('srs_socks_group'); ?>

                <div class="container p-4 mt-3">
                    <table class="table table-bordered table-striped" id="socks-table">
                        <thead class="table-dark">
                            <tr>
                                <th><?php _e('Sock Name', 'ski-ride-servetech'); ?></th>
                                    ...
                                    <tr class="gear-row" data-gear-index="<?php echo $gear_index; ?>">
                                        <td>
                                            <input type="text" class="form-control"
                                                name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][name]"
                                                value="<?php echo esc_attr($gear['name'] ?? ''); ?>">
                                            <input type="hidden"
                                                name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][product_id]"
                                                value="<?php echo esc_attr($gear['product_id'] ?? 0); ?>">
                                        </td>
                                        <td>
                                            <textarea class="form-control" rows="2"
                                                name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][desc]"><?php echo esc_textarea($gear['desc'] ?? ''); ?></textarea>
                                        </td>
                                        <td>
                                            <div class="form-check form-switch mb-2">
                                                <?php $is_rental = !empty($gear['is_rental']); ?>
                                                <input class="form-check-input rental-toggle" type="checkbox" role="switch"
                                                    id="so-rental-toggle-<?php echo $gear_index; ?>"
                                                    name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][is_rental]"
                                                    value="1" <?php checked($is_rental, true); ?>>
                                                <label class="form-check-label" for="so-rental-toggle-<?php echo $gear_index; ?>"><?php _e('Rental (multi-day pricing)', 'ski-ride-servetech'); ?></label>
                                            </div>
                                            ...
                                            <div class="base-price-wrapper mb-2" style="<?php echo $is_rental ? 'display:none;' : ''; ?>">
                                                ...
                                                <input type="number" step="0.01" class="form-control base-price-input"
                                                    name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][base_price]"
                                                            ...
                                                            <tr>
                                                                <td>
                                                                    <input type="number" min="1" class="form-control"
                                                                        name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][prices][day][]"
                                                                        value="<?php echo esc_attr($day); ?>">
                                                                </td>
                                                                <td>
                                                                    <input type="number" step="0.01" class="form-control"
                                                                        name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][prices][value][]"
                                        ...
                                        <td>
                                            <input type="number" step="0.01" class="form-control extra-price-input" <?php echo $is_rental ? '' : 'disabled'; ?>
                                                name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][prices][extra]"
                                        ...
                                        <td>
                                            <?php if (!empty($renting_options)): 
                                                ...
                                                ?>
                                                <select class="form-select" name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][renting_options][]" multiple>
                                        ...
                                        <td>
                                            <select class="form-select" name="<?php echo $this->option_key; ?>[socks][<?php echo $gear_index; ?>][type_options][]" multiple>
                                                <?php 
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-danger btn-sm remove-socks-row"><?php _e('Remove Sock', 'ski-ride-servetech'); ?></button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <button type="button" class="btn btn-success add-socks-row"><?php _e('Add Sock', 'ski-ride-servetech'); ?></button>
                </div>

                <div class="mt-3">
                    <?php submit_button(__('Save Socks', 'ski-ride-servetech')); ?>
                </div>
            </form>
        </div>
        <?php
    ...
    public function enqueue_scripts($hook) {
        if ($hook !== 'ski-ride-content_page_dev-ski-socks') return;

        $renting_options = get_option('srs_renting_options', []);

        wp_enqueue_script(
            'srs-socks-js',
            plugin_dir_url(__DIR__) . '../../assets/js/socks.js',
            ['jquery'],
            SRS_PLUGIN_VERSION,
            true
        );

        wp_localize_script('srs-socks-js', 'srsSocks', [
